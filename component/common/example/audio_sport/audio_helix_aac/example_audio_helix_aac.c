...
void audio_play_binary_array(uint8_t const *srcbuf, uint32_t len) {
    ...
    while (1) {
        ret = AACDecode(hAACDecoder, &inbuf, &bytesLeft, (short*)decodebuf);
...
void example_audio_helix_aac(void)
{
    ...
    if(xTaskCreate(example_audio_helix_aac_thread, "example_audio_helix_aac_thread", EXAMPLE_AUDIO_HELIX_AAC_HEAP_SIZE, NULL, tskIDLE_PRIORITY + 1, NULL)!= pdPASS)
      printf("\n\r%s xTaskCreate(example_audio_helix_aac_thread) failed", __FUNCTION__);
}
