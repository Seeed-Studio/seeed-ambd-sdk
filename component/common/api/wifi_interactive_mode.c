...
static void cmd_wifi_sta_and_ap(int argc, char **argv)
{
	...
	if (argc == 4) {
		if (wifi_start_ap(argv[1],
				  ...
				  strlen((const char *) argv[3]), channel) != RTW_SUCCESS) {
			printf("\n\rERROR: Operation failed!");
			return;
		}
	} else {
		if (wifi_start_ap(argv[1],
				  RTW_SECURITY_OPEN, NULL, strlen((const char *) argv[1]), 0, channel) != RTW_SUCCESS) {
			printf("\n\rERROR: Operation failed!");
			return;
		}
	}
...
static void cmd_wifi_connect_bssid(int argc, char **argv)
{
	...
	ret = wifi_connect_bssid(bssid,
				 ssid, security_type, password, bssid_len, ssid_len, password_len, key_id, semaphore);
...
static void cmd_debug(int argc, char **argv)
{
	if (strcmp(argv[1], "ready_trx") == 0) {
		...
		rtw_mac_t mac;
		sscanf(argv[2], MAC_FMT, (int *) (mac.octet + 0), (int *) (mac.octet + 1), (int *) (mac.octet + 2),
		       (int *) (mac.octet + 3), (int *) (mac.octet + 4), (int *) (mac.octet + 5));
...
void start_interactive_mode(void)
{
	...
	if (xTaskCreate
	    (interactive_mode, (char const *) "interactive_mode", STACKSIZE, NULL, tskIDLE_PRIORITY + 4,
	     NULL) != pdPASS)
